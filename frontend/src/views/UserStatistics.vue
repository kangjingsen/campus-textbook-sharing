<template>
  <div class="user-statistics">
      ...
      <div class="filter-row">
        ...
        <div class="actions">
          <el-checkbox-group v-model="selectedPanels">
            ...
            <el-checkbox-button label="funnel">转化漏斗</el-checkbox-button>
    ...
    <el-row :gutter="16" v-if="selectedPanels.includes('overview')" style="margin-top: 16px;">
      <el-col :span="4"><el-statistic title="我的教材数量" :value="overview.my_total_textbooks || 0" /></el-col>
      <el-col :span="4"><el-statistic title="在架教材" :value="overview.my_active_textbooks || 0" /></el-col>
      <el-col :span="4"><el-statistic title="积压数量" :value="overview.my_backlog_count || 0" /></el-col>
      <el-col :span="4"><el-statistic title="教材成交额" :value="overview.my_sales_amount || 0" prefix="¥" /></el-col>
      <el-col :span="4"><el-statistic title="我的资料数量" :value="overview.my_total_resources || 0" /></el-col>
      <el-col :span="4"><el-statistic title="资料成交额" :value="overview.my_resource_sales_amount || 0" prefix="¥" /></el-col>
    </el-row>

    <el-card v-if="selectedPanels.includes('funnel')" style="margin-top: 16px;" header="我的教材上架转化漏斗">
      <el-row :gutter="16">
        <el-col :span="5"><el-statistic title="在架教材" :value="textbookFunnel.listed || 0" /></el-col>
        <el-col :span="5"><el-statistic title="被下单" :value="textbookFunnel.ordered || 0" /></el-col>
        <el-col :span="5"><el-statistic title="已确认" :value="textbookFunnel.confirmed || 0" /></el-col>
        <el-col :span="5"><el-statistic title="已完成" :value="textbookFunnel.completed || 0" /></el-col>
        <el-col :span="4"><el-statistic title="已取消" :value="textbookFunnel.cancelled || 0" /></el-col>
      </el-row>
      <el-divider />
      <el-row :gutter="16">
        <el-col :span="12">
          <el-progress :percentage="orderedRate" :stroke-width="16" text-inside>
            <span>下单转化率 {{ orderedRate }}%</span>
          </el-progress>
        </el-col>
        <el-col :span="12">
          <el-progress :percentage="completedRate" status="success" :stroke-width="16" text-inside>
            <span>成交转化率 {{ completedRate }}%</span>
          </el-progress>
        </el-col>
      </el-row>
    </el-card>
      ...
      <el-col :span="12" v-if="selectedPanels.includes('backlog')">
        <el-card header="积压排行榜（可考虑不卖/下架）">
          <el-table :data="backlogRows" size="small" stripe>
            ...
            <el-table-column label="教材" min-width="180">
              <template #default="{ row }">
                <el-link type="primary" @click="goTextbookSearch(row.title)">{{ row.title }}</el-link>
              </template>
            </el-table-column>
            <el-table-column prop="same_title_count" label="同名数量" width="90" />
...
const selectedPanels = ref(['overview', 'funnel', 'backlog', 'demand', 'popular', 'sellers'])
const limit = ref(10)
const overview = ref({})
const textbookFunnel = ref({})
...
const orderedRate = computed(() => {
  const listed = Number(textbookFunnel.value.listed || 0)
  const ordered = Number(textbookFunnel.value.ordered || 0)
  if (!listed) return 0
  return Math.min(100, Math.round(ordered / listed * 100))
})

const completedRate = computed(() => {
  const listed = Number(textbookFunnel.value.listed || 0)
  const completed = Number(textbookFunnel.value.completed || 0)
  if (!listed) return 0
  return Math.min(100, Math.round(completed / listed * 100))
})
...
const loadData = async () => {
  try {
    ...
    textbookFunnel.value = res.data.textbook_funnel || {}
